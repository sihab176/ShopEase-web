...
export const authOptions = {
  ...
  callbacks: {
    // JWT token এর মধ্যে role attach করো
    async jwt({ token, user }) {
      if (user) {
        token.role = user.role || "user"; // user.role না থাকলে default user
      }
      return token;
    },

    // session এ role attach করো
    async session({ session, token }) {
      session.user.role = token.role || "user";
      return session;
    },

    // Google login এর জন্য signIn callback
    async signIn({ user, account }) {
      await dbConnect();

      if (account?.provider === "google") {
        const isExisted = await userModel.findOne({ email: user.email });
        if (!isExisted) {
