...
//         "http://localhost:3000/api/product/implement-func",
//         {
//           cache: "no-store",
//         }
//       );
...
const AllProductPage = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`http://localhost:3000/api/product/implement-func?${params.toString()}`, {
        cache: "no-store",
      });
      const data = await res.json();
      setProducts(data);
  ...
  useEffect(() => {
    fetchData();
  }, [search, category, brand, sort, maxPrice]);
    ...
    <div>
      ...
      <section className="grid grid-cols-12 max-w-7xl mx-auto gap-3.5 ">
        ...
        <div className="col-span-12 md:col-span-9 lg:col-span-10">
          ...
          <section className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 my-10 md:mx-0 mx-4">
            {products.length > 0 ? (
              products.map((product) => <Cards key={product._id} data={product} />)
            ) : (
